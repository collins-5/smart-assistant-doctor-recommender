...
function Tabs({ className, ...props }: RootProps) {
  return (
    <PrimitiveTabs
      className={cn(
        "mx-auto w-full max-w-[400px] flex-col items-center gap-1.5",
        className
      )}
      {...props}
    />
...
const VariantContext = React.createContext<VariantProps<
  typeof tabListVariants
> | null>(null);
...
const useVariantContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useVariantContext must be used within a VariantContextProvider "
    );
  }
